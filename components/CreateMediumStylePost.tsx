...
export default function CreateMediumStylePost({ session }: CreateMediumStylePostProps) {
  ...
  const [author, setAuthor] = useState(session?.user?.name || 'Default Author');
  const [authorImageUrl, setAuthorImageUrl] = useState(session?.user?.image || 'https://miro.medium.com/v2/resize:fill:40:40/0*zFTV8OpWZVwQRLXd');
